#This script is adapted from Spencer Schien (@MrPecners) to 
# produce low resolution country level population density maps.
#There seems to be a bug with the high definition codes. See the original
#codes here: https://spencerschien.info/post/data_viz_how_to/high_quality_rayshader_visuals/
...
# This script reads in data for Algeria

data <- st_read("00_data/kontur_population_DZ_20220630.gpkg")
...
#Add Annotationd with magick

 library(magick)

# Read in image, save to `img` object
img <- image_read("images/Algeria_pop_lowres.png")

# Set text color
text_color <- colors[4]

img_ <- image_annotate(img, "A Portrait of", font = "Felix Titling",
                       color = colors[1], size = 60, gravity = "north",
                       location = "+0+50")
# Subtitle
img_ <- image_annotate(img_, "Algeria Population", weight = 700, 
                       font = "Felix Titling", location = "+0+100",
                       color = text_color, size = 90, gravity = "north") 

 img_ <- image_annotate(img_, glue("Graphic by Pat Stephenson (@stepminer2) | ", 
                                  "Data from Kontur"), 
                       font = "Felix Titling", location = "+0+10",
                       color = alpha(text_color, .5), size = 20, gravity = "south")

image_write(img_, glue("images/algeria_fully_annotated.png")) 
