...
positions <- c(0.5, -0.5, 1.0, -1.0, 1.5, -1.5, 2.0, -2.0)
directions <- c(1, -1)
...
#Further cleanup to remove less significant storms

hurr_firsts_working <- hurr_firsts_working %>% 
 mutate(event1 = case_when(stringr::str_detect(event,"^Storm.*") ~ "NA"))   
 
 hurr_firsts_working1 <- print(hurr_firsts_working[!complete.cases(hurr_firsts_working), ])
 
# mutate event as factor 
hurr_firsts_working1 <-  hurr_firsts_working1 %>%
mutate (event= as.factor(event))  %>% 
 distinct()  

 #plot
timeline_plot<-ggplot(hurr_firsts_working1, aes(x=year, y = 0, col= event)) +
 ...
 geom_segment(data=hurr_firsts_working1[hurr_firsts_working1$year_count == 1,], aes(y=position,yend=0,xend=year), color='white', size=0.2) +
 ...
 #(data = hurr_firsts_working1, aes(y=text_position,label=event),size=4.5)+
 ggrepel::geom_text_repel(
      data =  hurr_firsts_working1,
      aes(y = text_position, label = event),
     max.overlaps = 100, size = 4.5 )
# ggrepel::geom_text_repel(
...
ggsave("HTI_hurricanes_storms.png",timeline_plot, width = 5, height = 6.5, dpi = 320, device = ragg::agg_png) 
